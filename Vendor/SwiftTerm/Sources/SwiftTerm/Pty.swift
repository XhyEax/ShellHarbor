...
public class PseudoTerminalHelpers {
    ...
    public static func fork (andExec: String, args: [String], env: [String], currentDirectory: String? = nil, desiredWindowSize: inout winsize) -> (pid: pid_t, masterFd: Int32)?
    {
        ...
        var currentDirectoryDescriptor: Int32 = -1
        if let currentDirectory {
            ...
            currentDirectoryDescriptor = open(duplicatedCurrentDirectory, O_RDONLY)
            guard currentDirectoryDescriptor >= 0 else {
                free(duplicatedCurrentDirectory)
                free(cExecutable)
                freeCStringArray(cEnv)
                freeCStringArray(cArgs)
                return nil
        ...
        defer {
            ...
            if currentDirectoryDescriptor >= 0 {
                close(currentDirectoryDescriptor)
            }
        }
        ...
        if pid == 0 {
            if currentDirectoryDescriptor >= 0,
                fchdir(currentDirectoryDescriptor) != 0
            {
                _exit(126)
            }

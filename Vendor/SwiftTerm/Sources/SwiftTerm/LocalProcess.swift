...
public class LocalProcess {
    ...
    // DispatchIO may deliver one read operation in many partial callbacks.
    // Keep delivery bounded without ever starting a second read operation:
    // the serial read queue waits when the main actor falls behind, and each
    // main-queue delivery releases one slot after SwiftTerm has consumed it.
    // This preserves the PTY byte stream exactly, which is essential for Mosh
    // screen diffs and cursor-addressed tmux redraws.
    private let pendingDeliverySlots = DispatchSemaphore(value: 64)
    ...
    private func deliverReceivedData(_ bytes: [UInt8]) {
        if usesMainQueue {
            pendingDeliverySlots.wait()
            let slots = pendingDeliverySlots
    ...
    func childProcessRead (done: Bool, data: DispatchData?, errno: Int32) {
        guard let data else {
            // A partial callback still belongs to the current read. Only its
            // final callback is allowed to start the successor operation.
            if done, running {
                resumePtyRead()
            }
            return
        ...
        deliverReceivedData(b)
        // Exactly one final callback starts exactly one successor. Blocking
        // above for an available delivery slot provides backpressure without
        // a separate resume path that can race this one.
        if done {
            resumePtyRead()
        }
    }

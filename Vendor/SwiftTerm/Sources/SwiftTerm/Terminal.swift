...
open class Terminal {
    ...
    public init (delegate: TerminalDelegate, options: TerminalOptions = TerminalOptions.default)
    {
        ...
        // Match iTerm2's "Scrollback in Alternate Screen" behavior so tmux
        // output can be reviewed locally without sending input to tmux.
...
//                break
    ...
    public func scroll (isWrapped: Bool = false)
    {
        ...
        var topRow = buffer.yBase + scrollTop
        var bottomRow = buffer.yBase + scrollBottom
        ...
        if hasNarrowMargins {
            ...
            // iTerm2's `useScrollbackWithRegion` preserves the line leaving a
            // partial-height scrolling region (tmux commonly reserves a top
            // or bottom status row). SwiftTerm previously discarded it,
            // leaving only a small fragment of tmux output in local history.
            if hasScrollback {
                let wasFull = lines.isFull
                lines.splice(
                    start: buffer.yBase,
                    deleteCount: 0,
                    items: [BufferLine(from: lines[topRow])],
                    change: { line in updateRange(line) }
                )
                if !wasFull {
                    buffer.yBase += 1
                    if !userScrolling { buffer.yDisp += 1 }
                } else {
                    buffer.linesTop += 1
                }
                topRow = buffer.yBase + scrollTop
                bottomRow = buffer.yBase + scrollBottom
            }
    ...
    func _getSelectedLines(_ start: Position, _ end: Position, buffer: Buffer) -> [Line]
    {
        ...
        while line < end.row {
            ...
            // Full-width lines are soft continuations even when a full-screen
            // program (notably tmux through Mosh) repaints rows and loses the
            // continuation row's isWrapped metadata.
            isWrapped = bufferLine.isWrapped ||
                buffer.lines[line - 1].getTrimmedLength() >= buffer.cols
        ...
        if end.row != start.row {
            ...
            if bufferLine.hasAnyContent () {
                ...
                isWrapped = bufferLine.isWrapped ||
                    buffer.lines[end.row - 1].getTrimmedLength() >= buffer.cols

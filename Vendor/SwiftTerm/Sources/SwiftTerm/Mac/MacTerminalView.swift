...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// Native control size for the terminal scrollbar.
    public var scrollerControlSize: NSControl.ControlSize = .regular {
        didSet {
            scroller?.controlSize = scrollerControlSize
            scroller?.constraints.first(where: {
                $0.firstAttribute == .width
            })?.constant = scrollerWidth
        }
    }

    /// Native knob appearance. `.light` provides a stable gray knob on the
    /// application's dark terminal background.
    public var scrollerKnobStyle: NSScroller.KnobStyle = .default {
        didSet { scroller?.knobStyle = scrollerKnobStyle }
    }
    ...
    func setupScroller()
    {
        ...
        scroller.scrollerStyle = scrollerStyle
        scroller.controlSize = scrollerControlSize
        scroller.knobStyle = scrollerKnobStyle
    ...
    open override func scrollWheel(with event: NSEvent) {
        ...
        // Multiplexers advertise mouse tracking through the terminal protocol.
        // tmux only does so after reading `mouse on` from tmux.conf (or an

...
extension TerminalView {
    ...
    func resetFont()
    {
        ...
        if (frame.width > 0) && (frame.height > 0) {
            // Use the same effective content width as an ordinary view resize
            // (not the full frame that also contains the legacy scroller), and
            // notify the PTY through sizeChanged.  Computing a different grid
            // here leaves the shell and emulator with different column counts;
            // zsh autosuggestions then wrap at the PTY edge while SwiftTerm
            // paints at another edge, leaving a duplicated prompt fragment on
            // the preceding row. iTerm2 likewise derives its session grid from
            // the scroll view's content size before updating the PTY.
    ...
    {
        guard let caretView else { return }
        let previousCaretFrame = caretView.frame
        ...
        // Mosh/tmux can move a steady layer-backed caret many times during a
        // single repaint. Explicitly redraw the old and new cells so the old
        // block cannot remain visible as a cursor-colored space.
        let previousDamage = previousCaretFrame.insetBy(dx: -1, dy: -1)
        let currentDamage = caretView.frame.insetBy(dx: -1, dy: -1)
        // updateDisplay already runs on the UI thread.  Deferring these
        // invalidations lets a fast interactive redraw (for example zsh's
        // autosuggestion repaint) advance the terminal again before the old
        // cursor cell is repaired.  The delayed damage can then paint a stale
        // block over a later matching substring such as another "./".  Mark
        // both cells dirty in this display transaction so AppKit coalesces
        // them with the row update that moved the caret.
        setNeedsDisplay(previousDamage)
        setNeedsDisplay(currentDamage)
    }
    ...
    /**
     * Returns the thumb size in proportion to the visible content. ShellHarbor
     * retains history for both normal and alternate buffers, so tmux/zellij
     * output remains locally reviewable when remote mouse reporting is off.
     */

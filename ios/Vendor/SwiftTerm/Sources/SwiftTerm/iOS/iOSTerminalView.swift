...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate {
    ...
    private var longPressSelectionOrigin: Position?
    private var longPressSelectionStart: Position?
    private var longPressSelectionEnd: Position?

    private func extendLongPressSelection(to hit: Position) {
        guard let origin = longPressSelectionOrigin,
              let initialStart = longPressSelectionStart,
              let initialEnd = longPressSelectionEnd else { return }
        selection.pivot = Position.compare(hit, origin) == .before ? initialEnd : initialStart
    ...
    private func autoScrollLongPressSelection(_ gestureRecognizer: UILongPressGestureRecognizer) {
        ...
        if nextOffset != contentOffset.y {
            contentOffset.y = nextOffset
            extendLongPressSelection(to: calculateTapHit(gesture: gestureRecognizer).grid)
        }
    }
    ...
    @objc func longPress (_ gestureRecognizer: UILongPressGestureRecognizer)
    {
        ...
        case .began:
            ...
            // Begin selection immediately.  Showing the edit menu before a
            // selection exists adds an unnecessary "Select" step on iOS.
            UIMenuController.shared.hideMenu()
            lastLongSelect = nil
            selection.selectWordOrExpression(at: hit, in: terminal.displayBuffer)
            selection.selectionMode = .character
            longPressSelectionOrigin = hit
            longPressSelectionStart = selection.start
            longPressSelectionEnd = selection.end
        ...
        case .changed:
            ...
            if visibleY < 0 || visibleY > bounds.height {
                startSelectionTimer { [weak self, weak gestureRecognizer] in
                    ...
                    self.autoScrollLongPressSelection(gestureRecognizer)
                }
            }
        case .ended:
            stopSelectionTimer()
            isScrollEnabled = true
            longPressSelectionOrigin = nil
            longPressSelectionStart = nil
            longPressSelectionEnd = nil

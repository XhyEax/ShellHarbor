...
extension NIOSSHPublicKey {
    ...
    var userAuthenticationAlgorithmPrefix: String.UTF8View {
        if case .custom(let key) = self.backingKey,
           let key = key as? NIOSSHUserAuthenticationKeyProtocol {
            return key.userAuthenticationAlgorithmPrefix.utf8
        }
        return self.keyPrefix
    }

    var userAuthenticationBlobPrefix: String.UTF8View {
        if case .custom(let key) = self.backingKey,
           let key = key as? NIOSSHUserAuthenticationKeyProtocol {
            return key.userAuthenticationBlobPrefix.utf8
        }
        return self.keyPrefix
    }
...
extension ByteBuffer {
    @discardableResult
    mutating func writeSSHUserAuthenticationKey(_ key: NIOSSHPublicKey) -> Int {
        var writtenBytes = self.writeSSHString(key.userAuthenticationBlobPrefix)
        writtenBytes += self.writeSSHHostKeyWithoutHeader(key)
        return writtenBytes
    }
